	...
	<template>
	<div class="e-img" :style="{width:width,height:height,fontSize:fontSize,paddingLeft:paddingLeft,paddingRight:paddingRight,paddingTop:paddingTop,paddingBottom:paddingBottom,marginLeft:marginLeft,marginRight:marginRight,marginTop:marginTop,marginBottom:marginBottom,borderLeft:borderLeft,borderRight:borderRight,borderTop:borderTop,borderBottom:borderBottom,backgroundColor:backgroundColor,backgroundImage:backgroundImage,backgroundRepeat:backgroundRepeat,backgroundPosition:backgroundPosition,backgroundSize:backgroundSize,borderRadius:borderRadius,boxShadow:boxShadow,color:color}">
    <div class="e-img-single" :style="{width:imageWidth,height:imageHeight,borderRadius:imageBorderRadius,marginRight:urls.length>1?'10px':'0px'}" v-for="(url,index) in urls">
        <img :src="url" >
    </div>
</div>
...
<script>
    ...
    computed:{
        urls:function(){
            if(this.url){
                return this.url.split(this.splitChar);
            }else{
                return [];
            }
    ...
    props:{
        url:{
            type:String,
            default:"http://oxwmr019d.bkt.clouddn.com/image.png",
            name:"图片地址",
            desc:"图片地址"
        },
        splitChar:{
